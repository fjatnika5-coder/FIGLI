...
-- FINAL + [PATCH v4] Hapus CaughtFishVisual reference, fix gameState global leak, safety patches
...
local publishFishCatch      = RepStorage:WaitForChild("PublishFishCatch", 30)
...
-- ═══════════════════════════════════════════════════════════════
-- NOTIFICATION MANAGER — load langsung tanpa lewat _G
-- ═══════════════════════════════════════════════════════════════
...
task.spawn(function()
	local ok, err = pcall(function()
		...
		if mgr:Initialize(gui) then
			...
			-- expose global buat sistem lain (quest, event, dll)
			...
			print("[FishingSystem] ✅ NotificationManager ready!")
		else
...
local function registerFishTool(fishId, tool)
	...
	if fishToolWatched[tool] then return end -- jangan pasang AncestryChanged dobel
...
local vfxSplashEvent  = RepStorage:FindFirstChild("VFXSplashEvent")
if not vfxSplashEvent then
	vfxSplashEvent        = Instance.new("RemoteEvent")
	vfxSplashEvent.Name   = "VFXSplashEvent"
	vfxSplashEvent.Parent = RepStorage
end
...
task.spawn(function()
	...
	if _G.GraphicsVFXEnabled == false then
		...
		print("[VFX] Splash VFX disabled by graphics setting (POTATO)")
	end
end)
...
local function cleanupPlayerVFX(playerId)
	if activeVFXPerPlayer[playerId] then
		...
		if emitter:IsA("ParticleEmitter") then
			...
			elseif emitCount and emitCount > 0 then
				task.delay(emitDelay, function()
					if emitter and emitter.Parent then emitter:Emit(emitCount) end
				end)
			else
				emitter.Enabled = true
			end
...
-- ═══════════════════════════════════════════════════════════════
-- GAME STATE — [PATCH v4] Fix: gameState harus local, bukan global
-- ═══════════════════════════════════════════════════════════════
local gameState = {
	casted            = false,
	sinker            = nil,
...
function RodManager:CleanupFishing()
	...
	if gameState.sinker then pcall(function() gameState.sinker:Destroy() end); gameState.sinker = nil end
...
local function startMasterLoop(char)
	...
	conn = RunService.Heartbeat:Connect(function(dt)
		...
		if gameState.isAutoFishing and RodManager.isEquipped
			...
			task.delay(0.4, function() performCast(power) end)
		end
	end)
...
local function showFishCatchNotification(fishData)
	...
	task.spawn(function()
		...
		local shakeOffset = 3
		local shakeDur    = 0.05
		local origXOffset = display.Position.X.Offset
		local shakeInfo   = TweenInfo.new(shakeDur, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
		for _ = 1, 2 do
			TweenService:Create(display, shakeInfo, {
				Position = UDim2.new(display.Position.X.Scale, origXOffset + shakeOffset,
					display.Position.Y.Scale, display.Position.Y.Offset)
			}):Play(); task.wait(shakeDur)
			TweenService:Create(display, shakeInfo, {
				Position = UDim2.new(display.Position.X.Scale, origXOffset - shakeOffset,
					display.Position.Y.Scale, display.Position.Y.Offset)
			}):Play(); task.wait(shakeDur)
		...
		local returnTween = TweenService:Create(display, shakeInfo, {
			Position = UDim2.new(display.Position.X.Scale, origXOffset,
				display.Position.Y.Scale, display.Position.Y.Offset)
		})
		returnTween:Play(); returnTween.Completed:Wait()
		task.wait(VISIBLE_DURATION)
		local slideOut = TweenService:Create(display, tweenOut, { Position = hiddenPosition })
		slideOut:Play()
...
fishCaughtResult.OnClientEvent:Connect(function(result)
	...
	if result.rarity == "Epic" or result.rarity == "Legendary"
		or result.rarity == "Mitos" or result.rarity == "Secret"
		or result.rarity == "Unknown" then
		pcall(function()
			publishFishCatch:FireServer(result.name, result.weight, result.rarity)
		end)
	end

	if result.wasUpgraded and result.originalRarity ~= result.rarity then
		task.delay(TWEEN_DURATION + 0.2, function()
			GUIManager:ShowNotification(
				string.format("Luck Boost! %s -> %s %s!", result.originalRarity, result.rarity, result.name),
				5, FishingConfig.GetRarityColor(result.rarity))
		end)
	end
...
local function runFishingSequence(taskId, splash)
	local function validate()
		return gameState.activeFishingTask == taskId
			and RodManager.isEquipped and RodManager.currentRod
			and RodManager:IsValidRod()
			and gameState.casted and gameState.sinker
	end
	...
	for _ = 1, 3 do
		if not validate() then cleanup(); return false end
		if gameState.sinker and gameState.casted and splashTemplate and isVFXAllowed() then
			local bubble = splashTemplate:Clone()
			if bubble then
				bubble.Parent = workspace
				if bubble:FindFirstChild("Caster") then bubble.Caster.Value = player.Name end
	...
	cleanupCastEvent:FireServer()
	GUIManager:SlideFishingFrameOut(function() updateMobileButtonText() end)
	return true
end
...
function performCast(power)
	...
	if not gameState.sinker then gameState.canCast = true; return end
...
-- ═══════════════════════════════════════════════════════════════
local MAX_OTHER_HOOKS           = 4
local CAST_DISTANCE_LIMIT       = 60
local DISTANCE_TOLERANCE        = 15
local OTHER_PHYSICS_INTERVAL    = 0.083
local OTHER_LAND_CHECK_INTERVAL = 0.08
...
local function cleanupOtherCast(caster)
	...
	if data.hook and data.hook.Parent then data.hook:Destroy() end
	...
	if activeVFXPerPlayer[userId] then
		local vfx = activeVFXPerPlayer[userId]
		if vfx and vfx.Parent then vfx:Destroy() end
		activeVFXPerPlayer[userId] = nil
	end
end
...
castReplicationEvent.OnClientEvent:Connect(function(caster, rodPos, vel, rodName, power, t0)
	...
	-- [PATCH v4] Safety cleanup turun dari 25 -> 15 detik
	local hookRef = hook
