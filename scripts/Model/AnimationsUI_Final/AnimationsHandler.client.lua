...
-- Pastikan Module TopbarPlus (Icon) ada di ReplicatedStorage
...
StopSyncBtn.Size = UDim2.new(0, 140, 0, 44) -- Default Size
...
----------------------------------------------------
-- DYNAMIC SCALE & TEXT LOGIC (HP EXTRA GEDE)
----------------------------------------------------
...
local function getDynamicScale()
	...
	-- 🔥 Base dibesarkan lagi (sebelumnya 0.72)
	local MOBILE_BASE = 0.78  

	local base
	if viewportY >= 1800 then
		base = 0.88 * MOBILE_BASE   -- iPad Pro
	elseif viewportY >= 1200 then
		base = 0.95 * MOBILE_BASE   -- Tablet
	elseif viewportY >= 900 then
		base = 1.05 * MOBILE_BASE   -- HP besar
	else
		base = 1.15 * MOBILE_BASE   -- HP kecil (PALING GEDE)
	end

	-- 🔥 Multiplier diperbesar dikit
	if shortestSide < 650 then
		return base * 1.45   -- HP kecil (KERASA NAIK)
	elseif shortestSide < 900 then
		return base * 1.28   -- HP besar
	else
		return base * 1.15   -- Tablet
	end
end

----------------------------------------------------
-- APPLY SCALE + EXTRA WIDTH (FIX)
local function updateScale()
	local scale = getDynamicScale()
	UIScale.Scale = scale

	-- 🔥 EXTRA WIDTH KHUSUS MOBILE (TANPA PINDAH POSISI)
	if IS_MOBILE then
		...
		if cam then
			...
			local widthBoost = 1
			if shortestSide < 650 then
				widthBoost = 1.18   -- HP kecil
			elseif shortestSide < 900 then
				widthBoost = 1.12   -- HP besar
			else
				widthBoost = 1.06   -- Tablet
			end

			-- ⚠️ HANYA UBAH SIZE, JANGAN POSITION
			Main.Size = UDim2.new(
				Main.Size.X.Scale * widthBoost,
				Main.Size.X.Offset,
				Main.Size.Y.Scale,
				Main.Size.Y.Offset
			)
...
if Shared then
	...
	AnimationsData = {{name="Example", animationId="rbxassetid://1"}}
end
...
task.spawn(function()
	loadFavorites()
	updateTabs()
	updateScale() -- Apply Scale Akhir
	checkSyncStatus() 
end)

----------------------------------------------------
-- 🔓 PUBLIC API (DIPANGGIL FASTBAR)
----------------------------------------------------
local AnimationsUI = {}
...
-- expose ke _G biar FastBar bisa panggil
