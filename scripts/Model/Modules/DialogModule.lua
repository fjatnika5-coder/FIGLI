...
-- Fungsi helper untuk mendapatkan UI reference terbaru
local function getDialogResponsesUI()
	local player = game.Players.LocalPlayer
	if not player or not player.PlayerGui then return nil end

	local dialog = player.PlayerGui:FindFirstChild("dialog")
	if not dialog then return nil end

	return dialog:FindFirstChild("dialogResponses")
end

-- Fungsi helper untuk mendapatkan torso/root part dari NPC (support R6 dan R15)
local function getNPCRootPart(npc)
	-- Prioritas: HumanoidRootPart > Torso (R6) > UpperTorso (R15)
...
function DialogModule.new(npcName, npc, prompt, animation)
	...
	self:initializeResponseButtons()
...
-- Fungsi untuk initialize response buttons (dipanggil setiap kali dibutuhkan)
function DialogModule:initializeResponseButtons()
	local DIALOG_RESPONSES_UI = getDialogResponsesUI()
	if not DIALOG_RESPONSES_UI then 
		warn("Dialog responses UI not found!")
		return 
	end

	local template = DIALOG_RESPONSES_UI:FindFirstChild("template")
	if template then
		-- Hapus response buttons yang sudah ada (kecuali template)
...
function DialogModule:triggerDialog(player, questionNumber)
	-- Pastikan response buttons sudah diinisialisasi dengan benar
	self:initializeResponseButtons()
	...
	task.spawn(function()
		...
		if not uiResponses then
			warn("Dialog responses UI not found during response display!")
			return
		end
		...
		for i, response in ipairs(dialog.responses) do
			...
			if not option then
				warn("Response option " .. i .. " not found!")
				continue
			end
		...
		while self.active do
			-- Dapatkan root part dari NPC (support R6 dan R15)
			local npcRoot = getNPCRootPart(self.npc)
			...
			if npcRoot and playerRoot then
				...
				-- Jika tidak ada root part, keluar dari dialog
				warn("NPC or Player root part not found!")
				self:hideGui()
				responseNum = 0
				break
			end
